...
if (empty($_SESSION['namauser']) and empty($_SESSION['passuser'])) {
  ...
  <center>Untuk mengakses modul, Anda harus login <br>";
    ...
    if (in_array('__select_all__supplier', $supplier_array) || empty($supplier_array)) {
        $supplier_filter = "AND penjualan.no_meja <> 0 ";
    } else {
        ...
        $supplier_filter = "AND penjualan.no_meja IN ($supplier_in)";
    }
    ...
    if (in_array('__select_all__barang', $barangsearch_array) || empty($barangsearch_array)) {
        $barangsearch_filter = "";
    } else {
...
?>
    ...
    <style type="text/css">
        ...
        #example_customer_sales_wrapper {
            width: 100%;
            overflow-x: auto;
        }

        #example_customer_sales {
            min-width: 1800px;
            width: 100%;
            table-layout: fixed;
        }
    ...
    <?php if ($tipe_laporan == 'summary') { ?>
        <div class="table-responsive">
            ...
            <table id="example_customer_sales" class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    $sql1 = mysqli_query($koneksi, "
                        SELECT 
                            COALESCE(m.nama, '-') AS nama_supplier,
                            penjualan.faktur as no_invoice,
                            penjualan.no_meja as kd_supp,
                        ...
                        WHERE penjualan.tanggal >= '$tgl_awal' 
                            ...
                            $barangsearch_filter 
                            $tipe_lokasi_filter
                        GROUP BY penjualan.no_meja, jualdetil.kd_brg
                    ...
                    ?>
                        ...
                        ?>
                            <tr align="left" style="background-color: #dee2e6;">
                                ...
                                <td><?php echo $invoice['kodebarang']; ?></td>
                                <td><?php echo $invoice['nama_barang']; ?></td>
                                <td style="font-weight: bold;" class="force-string"><a href="../../route/lap_baru/customer_sales_model.php?filter=semua&nilai=semua&supplier=<?php echo $array_new_supplier_string; ?>&tipe_laporan=detail&tgl_awal=<?php echo $tgl_awal; ?>&tgl_akhir=<?php echo $tgl_akhir; ?>&barangsearch=<?php echo $array_new_barang_string; ?>&tipe_lokasi=<?php echo $tipe_lokasi; ?>"><?php echo $invoice['jumlah_transaksi']; ?></td>
            ...
            <table id="example_customer_sales" class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    WHERE penjualan.tanggal >= '$tgl_awal' 
                        ...
                        $barangsearch_filter 
                        $tipe_lokasi_filter
                    ORDER BY m.nama, penjualan.tanggal ASC
                    ");
    ...
    <?php } else if ($tipe_laporan == 'custom') { ?>
        <div class="table-responsive">
            ...
            <table id="example_customer_sales" class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    $sql1 = mysqli_query($koneksi, "SELECT 
                        ...
                        COALESCE(m.telp, '-') AS Nomorktp,
                        COALESCE(m.alamat, '-') AS alamat,
                        COALESCE(m.kelurahan, '-') AS kelurahan,
                        COALESCE(m.kecamatan, '-') AS kecamatan,
                        COALESCE(m.kabupaten, '-') AS kabupaten,
                        COALESCE(m.provinsi, '-') AS provinsi,
                        jualdetil.kd_brg AS kodebarang,
                        barang.nama AS nama_barang,
                        (jualdetil.banyak * jualdetil.qty_satuan) AS quantity,
                        jualdetil.jumlah AS total_cost,
                        (COALESCE(mutasi_stok.harga_rata, 0) * jualdetil.banyak * jualdetil.qty_satuan) AS HPP
                    FROM penjualan
                    JOIN jualdetil 
                        ON jualdetil.faktur = penjualan.faktur 
                    LEFT JOIN barang 
                        ON barang.kd_brg = jualdetil.kd_brg
                    LEFT JOIN (
                        SELECT kd_member, 
                            MAX(nama) AS nama, 
                            MAX(telp) AS telp, 
                    ...
                    LEFT JOIN mutasi_stok 
                        ON mutasi_stok.kd_brg = jualdetil.kd_brg
                        AND mutasi_stok.tgl = DATE(penjualan.tanggal)
                        AND mutasi_stok.kd_cus = CASE WHEN penjualan.b_paking = 1 THEN 1316 ELSE 8001 END
                    WHERE penjualan.tanggal >= '$tgl_awal' 
                    AND penjualan.tanggal < DATE_ADD('$tgl_akhir', INTERVAL 1 DAY) AND (jualdetil.banyak * jualdetil.qty_satuan) > 0
                        $supplier_filter 
                        $barangsearch_filter 
                    ...
                    ?>
                        <tr align="left" style="background-color: #dee2e6;">
                            <td><?php echo $s1['nama_member']; ?></td>
                            <td><?php echo $s1['Nomorktp']; ?></td>
                            <td>
                                <?php
                            ...
                            <td class="bg1" align="right"><?php echo number_format($s1['HPP']); ?></td>
                            <td class="bg2" align="right"><?php echo number_format($s1['total_cost'] - $s1['HPP']); ?></td>
                            <td class="bg3" align="right"><?php echo number_format(($s1['HPP'] ?? 0) != 0 ? ($s1['total_cost'] - $s1['HPP']) / $s1['HPP'] * 100 : 0); ?>%</td>
                            <td class="bg3" align="right"><?php echo number_format(($s1['total_cost'] - $s1['HPP']) / $s1['total_cost'] * 100); ?>%</td>
                        </tr>
                    <?php
                    }
                    ?>
    ...
    <script>
        ...
        //   $('#example_customer_sales').DataTable({
        //     dom: 'Bfrtip',
        //     scrollX: true,
        //     buttons: [
