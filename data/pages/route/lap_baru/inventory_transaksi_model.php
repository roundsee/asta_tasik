...
if (empty($_SESSION['namauser']) and empty($_SESSION['passuser'])) {
  ...
  <center>Untuk mengakses modul, Anda harus login <br>";
    ...
    $judulform = "Inventory Transaction";
...
?>
    ...
    <style type="text/css">
        ...
        #example_inventory_transaction_wrapper {
            width: 100%;
            overflow-x: auto;
        }

        #example_inventory_transaction {
            min-width: 1800px;
            width: 100%;
            table-layout: fixed;
        }
    ...
    <?php if ($tipe_laporan == 'summary') { ?>
        <div class="table-responsive">
            <table id="example_inventory_transaction" class="table table-bordered table-striped">
                <thead style="background-color: lightgray;" class="elevation-2">
                    <tr>
                        ...
                        <th>Quantity OUT</th>
                        <th>Unit</th>
                        <th class="bg1">Amt IN</th>
                        <th class="bg1">Amt OUT</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    ...
                    $sql1 = mysqli_query($koneksi, "SELECT 
                        ...
                        'Sales' as type_transaksi,
                        barang.Satuan1 AS unit,
                        sum(jualdetil.banyak * jualdetil.qty_satuan) AS quantity_out,
                        0 as quantity_in,
                        sum(jualdetil.jumlah) AS amt_out,
                        0 as amt_in
                    FROM jualdetil
                    LEFT JOIN barang ON barang.kd_brg = jualdetil.kd_brg
                    WHERE jualdetil.tanggal >= '$tgl_awal' 
                        AND jualdetil.tanggal <= DATE_ADD('$tgl_akhir', INTERVAL 1 DAY)
                        $barangsearch_filter
                    ...
                    if (in_array('__select_all__', $barangsearch_array) || empty($barangsearch_array)) {
                        ...
                        $barangsearch_filter = "AND pembelian_invoice_detail.kd_brg IN ($barangsearch_in)";
                    }
                    $sql2 = mysqli_query($koneksi, "SELECT 
                        barang.nama as nama_barang,
                        pembelian_invoice_detail.kd_brg as kodebarang, 
                        COUNT(*) AS jumlah_transaksi,
                        'Purchases' as type_transaksi,
                        barang.Satuan1 AS unit,
                        SUM(pembelian_invoice_detail.jml * pembelian_invoice_detail.jml_pcs) as quantity_in,
                        0 as quantity_out,
                        SUM(pembelian_invoice_detail.nilai * pembelian_invoice_detail.jml) AS amt_in,
                        0 as amt_out
                        FROM pembelian_invoice_detail
                        JOIN pembelian_invoice ON pembelian_invoice.no_invoice = pembelian_invoice_detail.no_invoice
                        LEFT JOIN barang ON barang.kd_brg = pembelian_invoice_detail.kd_brg
                        WHERE pembelian_invoice.tanggal_invoice BETWEEN '$tgl_awal' AND '$tgl_akhir' $barangsearch_filter
                        GROUP BY pembelian_invoice_detail.kd_brg
                        ORDER BY pembelian_invoice_detail.kd_brg ASC;");

                    if (!$sql2) {
                        die("Query gagal: " . mysqli_error($koneksi));
                    }
                    while ($s2 = mysqli_fetch_array($sql2)) {
                        $supplier = $s2['kodebarang'];
                        if (!isset($suppliers_data[$supplier])) {
                            $suppliers_data[$supplier] = [];
                        }
                        $suppliers_data[$supplier][] = $s2;
                    }
                    $grandtotalin = 0;
                    $grandtotalout = 0;
                    ...
                    foreach ($suppliers_data as $supplier_name => $invoices) {
                        $totaljumlahhargain = 0;
                        $totaljumlahhargaout = 0;
                    ?>
                        ...
                        foreach ($invoices as $invoice) {
                            $grandtotalin += $invoice['amt_in'];
                            $grandtotalout += $invoice['amt_out'];
                            $totaljumlahhargain += $invoice['amt_in'];
                            $totaljumlahhargaout += $invoice['amt_out'];
                            $array_BarangData = [];
                            $array_BarangData[] = $invoice['kodebarang'];
                        ...
                        ?>
                            <tr align="left" style="background-color: #dee2e6;">
                                ...
                                <td><?php echo $invoice['type_transaksi']; ?></td>
                                <td style="font-weight: bold;" class="force-string"><a href="../../route/lap_baru/inventory_transaksi_model.php?filter=semua&nilai=semua&tipe_laporan=detail&tgl_awal=<?php echo $tgl_awal; ?>&tgl_akhir=<?php echo $tgl_akhir; ?>&barangsearch=<?php echo $array_new_barang_string; ?>"><?php echo $invoice['jumlah_transaksi']; ?></td>
                                <td align="right"><?php echo number_format($invoice['quantity_in']); ?></td>
                                <td align="right"><?php echo number_format($invoice['quantity_out']); ?></td>
                                <td><?php echo $invoice['unit']; ?></td>
                                <td class="bg1" align="right"><?php echo number_format($invoice['amt_in']); ?></td>
                                <td class="bg1" align="right"><?php echo number_format($invoice['amt_out']); ?></td>
                            </tr>
                        <?php
                        }
                        ?>
                        <tr align="left" style="background-color: #e9ecef;">
                            ...
                            <td></td>
                            <td></td>
                            <td class="bg1" align="right"><?php echo number_format($totaljumlahhargain); ?></td>
                            <td class="bg1" align="right"><?php echo number_format($totaljumlahhargaout); ?></td>
                        </tr>
                    <?php
                    }
                    ?>
            ...
            <!-- <h1><?php echo $grandtotalin; ?></h1>
            <h1><?php echo $grandtotalout; ?></h1> -->
        </div>
        ...
        <div class="table-responsive">
            <table id="example_inventory_transaction" class="table table-bordered table-striped">
                ...
                <tbody>
                    ...
                    $sql1 = mysqli_query($koneksi, "SELECT 
                            barang.nama AS nama_barang,
                            DATE(jualdetil.tanggal) as tanggal_invoice,
                            jualdetil.kd_brg AS kodebarang,
                            jualdetil.faktur as invoice,
                            'SAL' as type_transaksi,
                            barang.Satuan1 AS unit,
                            (jualdetil.banyak * jualdetil.qty_satuan) AS quantity_out,
                            0 as quantity_in,
                            jualdetil.jumlah AS amt_out,
                            0 as amt_in,0 as kd_beli
                        FROM jualdetil
                        LEFT JOIN barang ON barang.kd_brg = jualdetil.kd_brg
                        WHERE jualdetil.tanggal >= '$tgl_awal' 
                            AND jualdetil.tanggal <= DATE_ADD('$tgl_akhir', INTERVAL 1 DAY)
                            $barangsearch_filter
                        ORDER BY jualdetil.kd_brg,jualdetil.tanggal ASC
                        ");
                    ...
                    $sql2 = mysqli_query($koneksi, "SELECT 
                        barang.nama as nama_barang,
                        pembelian_invoice.tanggal_invoice as tanggal_invoice,
                        pembelian_invoice_detail.kd_brg as kodebarang, 
                        pembelian_invoice_detail.no_invoice as invoice, 
                        'PUR' as type_transaksi,
                        barang.Satuan1 AS unit,
                        (pembelian_invoice_detail.jml * pembelian_invoice_detail.jml_pcs) as quantity_in,
                        0 as quantity_out,
                        (pembelian_invoice_detail.nilai * pembelian_invoice_detail.jml) AS amt_in,
                        0 as amt_out,pembelian.kd_beli
                        FROM pembelian_invoice_detail
                        JOIN pembelian_invoice ON pembelian_invoice.no_invoice = pembelian_invoice_detail.no_invoice
                        LEFT JOIN barang ON barang.kd_brg = pembelian_invoice_detail.kd_brg
                        LEFT JOIN pembelian ON pembelian.kd_po = pembelian_invoice_detail.kd_po 
                        WHERE pembelian_invoice.tanggal_invoice BETWEEN '$tgl_awal' AND '$tgl_akhir' $barangsearch_filter
                        ORDER BY pembelian_invoice_detail.kd_brg,pembelian_invoice.tanggal_invoice ASC;");
                    ...
                    // Sort each supplier's transactions by tanggal_invoice (ascending)
                    foreach ($suppliers_data as $supplier => &$transactions) {
                        usort($transactions, function ($a, $b) {
                            return strtotime($a['tanggal_invoice']) - strtotime($b['tanggal_invoice']);
                        });
                    }
                    unset($transactions); // Break the reference to avoid potential issues
                    $grandtotalin = 0;
                    $grandtotalout = 0;
                    ...
                    ?>
                        ...
                        ?>
                            <tr align="left" style="background-color: #dee2e6;">
                                ...
                                <td><?php echo $invoice['type_transaksi']; ?></td>
                                <?php if ($invoice['type_transaksi'] == 'SAL') {
                                    $link = "../../main.php?route=lap_pb1_detil_view&act&id={$invoice['invoice']}&asal=pb1";
                                ?>
                                    <td style="font-weight: bold;" class="force-string"><a href="<?php echo $link; ?>" title="Detail"><?php echo $invoice['invoice']; ?></a></td>
                                <?php
                                } else { ?>
                                    <td style="font-weight: bold;" class="force-string"><a href="../../main.php?route=purchase_order_detail&act&id=<?php echo $invoice['kd_beli']; ?>&asal=purchase_order" title="Detail"><?php echo $invoice['invoice']; ?></a></td>
                                <?php } ?>
                                <td align="right"><?php echo number_format($invoice['quantity_in']); ?></td>
                                <td align="right"><?php echo number_format($invoice['quantity_out']); ?></td>
                                <td><?php echo $invoice['unit']; ?></td>
    ...
    <script>
        ...
        //   $('#example_inventory_transaction').DataTable({
        //     dom: 'Bfrtip',
        //     scrollX: true,
        //     buttons: [
