...
<body style='font-family:tahoma; font-size:9pt;' onload="printOut()">
    ...
    $querypenjualan = mysqli_query($koneksi, "SELECT oleh,no_meja,tanggal,dibayar,jumlah,tarif_pb1,byr_pocer,byr_non_tunai,ongkir,voucher_nilai_diskon,
                                                COALESCE(
                                                    (SELECT member.nama
                                                    FROM member 
                                                    WHERE penjualan.no_meja = member.kd_member), 
                                                    0
                                                ) AS namamember,
    ...
    $nama_member = (isset($stppenjualan['namamember']) && trim($stppenjualan['namamember']) !== "" && $stppenjualan['namamember'] != 0) ? $stppenjualan['namamember'] : "-";
    ?>
    ...
    <script>
        ...
        function printOut() {
            ...
            t = setTimeout(function() {
                window.location.href = "dashboard_gudang.php";
            }, lama);
        };
    </script>
