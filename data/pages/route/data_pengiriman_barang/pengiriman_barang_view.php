...
if (empty($_SESSION['username']) and empty($_SESSION['passuser'])) {
	...
	switch ($_GET['act']) {
		default:
			...
			SELECT $tabel.*, 
				   IFNULL(pbi.kode_pengiriman, '0') AS kode_pengiriman_status 
			FROM $tabel 
											...
											<div class="box-body">
																...
																<div class="col-lg-4">
																	<div class="form-group">
																		...
																		<select name="<?php echo $f5; ?>" class="form-control" disabled>
																			<option value="1" <?= ($status_permintaan == 1) ? 'selected' : ''; ?>>Proses</option>
																			<option value="2" <?= ($status_permintaan == 2) ? 'selected' : ''; ?>>Selesai</option>
																			<option value="0" <?= ($status_permintaan == 0) ? 'selected' : ''; ?>>Belum melakukan pengiriman</option>
																		</select>
																	</div>
																</div>
												...
												<table id="example1" width="100%" border="1" cellspacing="0" cellpadding="0" class="table table-bordered table-striped">
													...
													<tbody>
														...
														from pengiriman_barang_detail_internal pd
														LEFT JOIN employee ON employee.employee_number = pd.user_kirim
													 	JOIN barang b ON b.kd_brg=pd.kd_barang
														JOIN pengiriman_barang_internal ON pengiriman_barang_internal.kode_pengiriman = pd.kode_pengiriman
														where pengiriman_barang_internal.kode_permintaan='$_GET[id]'
														ORDER BY pd.user_kirim DESC  ");
