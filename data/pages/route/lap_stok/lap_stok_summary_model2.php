...
?>
  ...
  <section class="content-header  wow fadeInDown" data-wow-duration=".3s" data-wow-delay=".3s">
    <div class="container-fluid">
      ...
      <?php function formatMoney($amount)
      {
  ...
  <?php if ($filter == 'semua') { ?>
    <table id="example4" class="table table-bordered table-striped">
      ...
      SELECT 
        ...
        IFNULL(inv1.stok, '0') AS stok_swalayan,
        IFNULL(inv2.stok, '0') AS stok_gudang,
        b.Satuan1 AS satuan,
    ...
    LEFT JOIN inventory inv1 
        ON inv1.kd_brg = b.kd_brg AND inv1.kd_cus = '1316'
    LEFT JOIN inventory inv2 
        ON inv2.kd_brg = b.kd_brg AND inv2.kd_cus = '8001'
    LEFT JOIN (
  ...
  <?php } elseif ($filter == 'supplier') { ?>
    ...
    FROM 
        barang b
    LEFT JOIN inventory inv1 
        ON inv1.kd_brg = b.kd_brg AND inv1.kd_cus = '$nilai'
    LEFT JOIN (
        SELECT kd_brg, MAX(tgl) AS tgl
        FROM mutasi_stok
        WHERE kd_cus = '$nilai' AND qt_opname_hari != 0
        GROUP BY kd_brg
    ) ms1 ON ms1.kd_brg = b.kd_brg
    LEFT JOIN (
        SELECT ms.kd_brg, ms.harga_rata
        ...
        INNER JOIN (
            SELECT kd_brg, MAX(tgl) AS max_tgl
            FROM mutasi_stok
            WHERE kd_cus= '$nilai' AND harga_rata != 0
            GROUP BY kd_brg
        ) latest ON ms.kd_brg = latest.kd_brg AND ms.tgl = latest.max_tgl AND ms.kd_cus= '$nilai'
    ) mhpp1 ON mhpp1.kd_brg = b.kd_brg
    LEFT JOIN (
        SELECT pid1.kd_brg, (pid1.nilai / pid1.jml_pcs) as nilai2
        FROM pembelian_invoice_detail pid1
