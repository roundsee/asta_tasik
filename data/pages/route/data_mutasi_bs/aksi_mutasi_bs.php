...
if (empty($_SESSION['username']) and empty($_SESSION['passuser'])) {
	...
	elseif ($route == $tujuan and $act == 'input') {
		write_log("pengajuan: " . $_POST['no_pengajuan']);

		$tgl = date('ymd');
		$no_pengajuan = 'MBS-' . $_POST['no_pengajuan'];
		// echo $no_pengajuan;

		$kd_acc = $_POST['kd_acc'];
		write_log("kd_acc: " . $kd_acc);
		...
		$query = mysqli_query($koneksi, "SELECT max(id) as kodeTerbesar FROM mutasi_bs where left(no_pengajuan,13)='$no_pengajuan' ");
	...
	elseif ($route == $tujuan and $act == 'input-baru') {
		...
		$query = "INSERT INTO $tabel ( $f1, $f2, $f3)
		VALUES (
			'$kode_permintaan',
			'$_POST[$f2]',
			'$_POST[$f3]'
