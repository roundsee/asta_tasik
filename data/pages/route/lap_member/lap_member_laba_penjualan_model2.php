...
if ($members_list != " " && !empty($members_list)) {
    ...
    $kondisi4 = " AND penjualan.no_meja != 0";
}
...
<table id="example4" class="table table-bordered table-striped">
    ...
    <tbody>
        ...
        // $query = "
        //     SELECT $tabel.kd_brg, 
        //         barang.nama AS nama_barang, aggregated_jualdetil.membertes as member,
        //         aggregated_jualdetil.total_qty AS sum_qt_jual,
        //         aggregated_jualdetil.totaljumlah AS sum_nilai_jual,
        //             aggregated_jualdetil.total_qty * 
        //             (CASE WHEN sum($tabel.qt_jual) = 0 THEN 0 ELSE sum($tabel.hpp_jual / $tabel.qt_jual) END)
        //             AS sum_hpp_jual
        //     FROM $tabel
        //     JOIN barang ON barang.kd_brg = $tabel.kd_brg
        //     JOIN (
        //         SELECT kd_brg, SUM(banyak * qty_satuan) AS total_qty,  sum(jualdetil.jumlah) as totaljumlah,
        //         ifnull((SELECT nama from member where member.kd_member = penjualan.no_meja LIMIT 1),' ') as membertes
        //         FROM jualdetil
        //         JOIN penjualan ON penjualan.faktur = jualdetil.faktur
        //         WHERE jualdetil.tanggal between '$tgl_awal' and '$tgl_akhir'  +interval 1 day $kondisi4 
        //         GROUP BY kd_brg
        ...
        //     WHERE tgl = '$tgl_akhir' $kondisi3
        //     $kondisi
        //     GROUP BY $tabel.kd_brg
        //     ORDER BY $tabel.kd_brg ASC";
        $query = "
            SELECT jualdetil.kd_brg, SUM(banyak * qty_satuan) AS sum_qt_jual,  sum(jualdetil.jumlah) as sum_nilai_jual,
                ifnull((SELECT nama from member where member.kd_member = penjualan.no_meja LIMIT 1),' ') as member,
                barang.nama AS nama_barang,
                SUM(banyak * qty_satuan) * 
                ((SELECT (mutasi_stok.hpp_jual / mutasi_stok.qt_jual) as tes from mutasi_stok where 
                mutasi_stok.kd_brg = jualdetil.kd_brg AND mutasi_stok.hpp_jual !=0 ORDER BY tgl DESC LIMIT 1))
                AS sum_hpp_jual
                FROM jualdetil
                JOIN penjualan ON penjualan.faktur = jualdetil.faktur
                JOIN barang ON barang.kd_brg = jualdetil.kd_brg
                WHERE jualdetil.tanggal between '$tgl_awal' and '$tgl_akhir'  +interval 1 day $kondisi4 $kondisi3
                $kondisi
