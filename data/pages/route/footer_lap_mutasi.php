...
<script>
  ...
  $(function() {
    ...
    // Append buttons
    table.buttons().container().appendTo('#example4_wrapper .col-md-6:eq(0)');

    // Fix for zoom/resize: trigger re-layout
    $(window).on('resize', function() {
      table.columns.adjust().draw();
    });

    // Optional: adjust after zoom using a MutationObserver
    new ResizeObserver(() => {
      table.columns.adjust().draw();
